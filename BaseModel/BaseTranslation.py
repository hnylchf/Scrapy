...
class BaseTranslation():
    ...
    def __init__(self,query,foLang = 'zh',toLang='en'):
    ...
    def send(self):
        text_list = self.replaceImgs(self.q)
        translation_text = ''
        for t in text_list:
            if len(t) == 2:
                text = t[0]
                img = t[1]
                t = self.send_translation(text, self.foLang, self.toLang)
                translation_text = translation_text + self.send_translation(t,self.toLang,self.foLang) + img


            else:
                t = self.send_translation(text, self.foLang, self.toLang)
                translation_text = translation_text + self.send_translation(t, self.toLang, self.foLang)
...
　　[img]../images/36bfdca017d47b2164a54b7e769b50d1_cfb37c34b980a9266e664ec2d2244d582.html[/img]
M奥丁简介
