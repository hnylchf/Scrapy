...
class FileUtil():
    ...
    def initPath(self):
        ...
        if not os.path.exists(self.start_path + self.path):
            os.mkdir(self.start_path + self.path)
    ...
    #write_type = w(默认)新文件 a=》追加内容
    def write(self, path, str , write_type = 'w'):
        file_object = open(path, write_type)
    ...
    def saveContent(self, file_name, html):
        logger.info('save:' + self.content + file_name)
